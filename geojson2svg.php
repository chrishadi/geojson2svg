<?php
function geojson2svg ( $filename, $options ) {
...
function render_feature ( $feature, $options ) {
	// This is a bucket of 5-grades yellow to red color scheme from colorbrewer2.org
	...
	switch ( $geometry[ 'type' ] ) {
		...
		case 'MultiPolygon':
		foreach ( $coordinates as $polygon ) {
			return render_polygon ( $polygon, $color, $text, $options );
		}
	}
}
